...
def build_experiment_folder(experiment_name, dp=False):
    experiment_path = os.path.abspath(experiment_name)
    if dp:
        experiment_path = "{}/{}".format(experiment_path, 'dp_maml')
    else:
        experiment_path = "{}/{}".format(experiment_path, 'maml')
...
def update_json_experiment_log_dict(key, value, experiment_log_dir, log_name="experiment_log.json"):
    ...
    if not isinstance(summary_dict[key], list):
        summary_dict[key] = [summary_dict[key]]
    if not isinstance(value, list):
        summary_dict[key].append(value)
    else:
        summary_dict[key].extend(value)
